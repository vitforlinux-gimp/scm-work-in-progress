...
;
			...
			(if (or (= useFirstPointArrowAsHead 1) (= useDoubleHeadArrow 1))
				(if (= CurvedArrowhead 1)
					...
					(begin
                		(script-fu-help-1ArrowV3plus inPoint_1X inPoint_1Y Bezier_x Bezier_y theWingLength (* (/ WingAngle 180) pi) drawable image FullHead MiddlePoint)
					)	; end - begin
				) ; end - if
			) ; end - if
            ...
            (if (or (= useFirstPointArrowAsHead 0) (= useDoubleHeadArrow 1))
				(if (= CurvedArrowhead 1)
					...
					(begin
                		(script-fu-help-1ArrowV3plus inPoint_2X inPoint_2Y Bezier_x Bezier_y theWingLength (* (/ WingAngle 180) pi) drawable image FullHead MiddlePoint)
					) ; end - begin
				) ; end - if
			) ; end - if
...
(script-fu-register
    ...
    SF-COLOR       "Fill Color"         '(255 0 0)     ;color variable
    SF-ADJUSTMENT  "Length of wings (LoW)" '(20.0 1 500 1 10 1 0)
    ...
    SF-ADJUSTMENT  "Percentage size of notch of arrow head\n(only valid if head of arrow is filled)" '(75 0 100 1 10 0 0)
    SF-ADJUSTMENT  "Brush thickness (BT)" '(6 1 500 1 10 0 0)
    ...
    SF-TOGGLE      "Curved arrow wings? (only for curved paths)" TRUE
    SF-ADJUSTMENT  "Points for curved arrow wing (2 to 99)" '(20 2 99 1 10 0 0)
)
(script-fu-menu-register "script-fu-draw-arrowV3plus" "<Image>/Tools/Arrow...")
