...
(gimp-message "280 alpha")	
	...
	(set! bkg-layer (if (= (string->number (substring (car(gimp-version)) 0 3)) 2.1)  
 		(car (gimp-image-get-active-layer image))	
        	(vector-ref (car (gimp-image-get-selected-layers image)) 0)))	
...
;;;;set the different metal types
	...
	(set! image-layer (if (= (string->number (substring (car(gimp-version)) 0 3)) 2.1)  
	(car (gimp-image-get-active-layer image))	
	(vector-ref (car (gimp-image-get-selected-layers image)) 0)))
    (gimp-selection-none image)	
